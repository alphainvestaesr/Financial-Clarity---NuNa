...
function renderDados(){
  ...
  el('dd-sessao').innerHTML = s
    ? 'Conectada como <b>'+esc(s.nome)+'</b> &middot; a sess&atilde;o expira em '+new Date(s.expiraEm).toLocaleString('pt-BR')+'.'
    : '';
  ...
  el('dd-armazenamento').innerHTML = Store.disponivel
    ? '&#10003; Armazenamento do navegador ativo. Tudo que voc&ecirc; edita ou lan&ccedil;a continua aqui depois de recarregar, fechar a aba ou voltar amanh&atilde;.'
    : '<b>Aten&ccedil;&atilde;o:</b> o navegador est&aacute; bloqueando o armazenamento local (janela an&ocirc;nima ou cookies bloqueados). Nada ser&aacute; salvo ao fechar a aba.';
  renderFechamentos();
}
...
function wireDados(){
  ...
  el('dd-limpar').addEventListener('click', function(){
    ...
    [K.OVERRIDES,K.ORCAMENTOS,K.GASTEI,K.IMPORTADOS,K.FECHAMENTOS,K.TRANSF,K.PREFS].forEach(Store.remove);
    location.reload();
  });
  el('dd-sair').addEventListener('click', function(){ Auth.sair(); location.reload(); });
}
