...
function renderInsights(){
  ...
  if(state.perfil==='NuNa'){
    ...
    var d=Math.abs(ta-tm);
    wins.push('Nos meses fechados, <b>'+(ta>tm?'Ana':'Manuela')+'</b> contribuiu '+brl(d)+' a mais. Para equilibrar 50/50 no acumulado, faltaria repassar '+brl(d/2)+'.');
  }
...
function renderAll(skipTables){
  if(!DATA) return;
  var fns=[renderOverviewMonths,renderKPI,renderDonut,renderTransfers,renderSavingsTracker,renderOverviewInsight,
           renderTxMonthPills,renderTxInsight,renderMonthVsMonth,renderSaldos,renderContrib,renderCategoryTrend,renderMvMInsight,
           renderBudgetTable,renderBudgetInsight,renderInsights,renderInsightsFooter,agRenderCTA,renderDados,
  ...
  if(!skipTables) fns.splice(7,0,renderTxTable,renderReview);
  else fns.push(function atualizaPin(){var p=allTx().filter(function(t){return t.revisar}).length;
    el('rev-pin').textContent=p; el('rev-pin').dataset.zero=p===0?'1':'0';});
  fns.forEach(function(f){try{f()}catch(e){console.error('Erro em '+(f.name||'anon')+':',e)}});
}

/* ---------- inicializacao ---------- */
function mostrarTela(nome){
  ['landing','app'].forEach(function(t){ var n=el('tela-'+t); if(n) n.hidden = (t!==nome); });
  document.body.dataset.tela = nome;
}
function iniciarDashboard(){
  return fetch('data/base.json', {cache:'no-cache'})
    .then(function(r){ if(!r.ok) throw new Error('HTTP '+r.status); return r.json(); })
    .then(function(base){
      aplicarBase(base);
      var s=Auth.sessao(); if(s && s.perfil && !Store.get(K.PREFS,{}).perfil) state.perfil=s.perfil;
      [wireProfile,wireTabs,wireAxis,wireTxControls,wireReview,wireRefreshButton,agWire,wireDados]
    ...
    .catch(function(err){
      el('boot').innerHTML='<div class="boot-erro"><b>Nao consegui carregar os dados.</b><br>'+
        ...
        'Abrir o index.html direto do disco bloqueia a leitura de <code>data/base.json</code>.<br>'+
...
function wireLogin(){
  ...
  form.addEventListener('submit', function(e){
    e.preventDefault();
    var u=el('login-user').value, p=el('login-pass').value;
    var s=Auth.entrar(u,p);
    var erro=el('login-erro');
    if(!s){
      erro.textContent='Usuario ou senha incorretos.';
      erro.hidden=false; form.classList.remove('shake'); void form.offsetWidth; form.classList.add('shake');
      el('login-pass').value=''; el('login-pass').focus();
      return;
    }
    erro.hidden=true; el('login-pass').value='';
    abrirDashboard();
  });
}
(function boot(){
  ...
  document.addEventListener('DOMContentLoaded', function(){
    ...
    try{ wireLogin() }catch(e){ console.error('wireLogin',e) }
    if(Auth.sessao()) abrirDashboard(); else mostrarTela('landing');
  });
})();
